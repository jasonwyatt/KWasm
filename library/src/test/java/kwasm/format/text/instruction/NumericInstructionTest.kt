...
class NumericInstructionTest(private val testCase: Pair<String, NumericInstruction>) {
    @Test
    fun parsesInstruction() {
        val tokenizer = Tokenizer()
        val context = ParseContext("${testCase.first}.wat")
        assertThat(tokenizer.tokenize(testCase.first, context).parseNumericInstruction(0))
    ...
    fun parsesInstruction_fromParseInstruction() {
        val tokenizer = Tokenizer()
        val context = ParseContext("${testCase.first}.wat")
        assertThat(tokenizer.tokenize(testCase.first, context).parseInstruction(0))
            .isEqualTo(ParseResult(testCase.second, 1))
    }
    ...
    companion object {
        ...
        fun parameters(): List<Pair<String, NumericInstruction>> = listOf(
