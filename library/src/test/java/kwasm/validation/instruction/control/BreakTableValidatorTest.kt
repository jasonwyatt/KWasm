...
class BreakTableValidatorTest {
    @get:Rule
    val parser = ParseRule()

    @Test
    fun invalid_ifDefaultLabel_doesntExist() = parser.with {
        assertThrows(ValidationException::class.java) {
            "br_table 0".parseInstruction().validate(EMPTY_FUNCTION_BODY)
        }

        assertThrows(ValidationException::class.java) {
            "br_table \$foo".parseInstruction().validate(EMPTY_FUNCTION_BODY)
        }
    }

    @Test
    fun invalid_ifSecondaryLabels_dontExist() = parser.with {
        assertThrows(ValidationException::class.java) {
            "br_table 1 3 0".parseInstruction().validate(BODY_WITH_LABEL)
        }
    }

    @Test
    fun invalid_ifSecondaryLabels_dontMatchDefaultLabelType() = parser.with {
        assertThrows(ValidationException::class.java) {
            "br_table 1 0".parseInstruction()
                .validate(
                    BODY_WITH_LABEL
                        .prependLabel(ResultType(Result(ValueType.I64)))
                )
        }.also {
            assertThat(it).hasMessageThat()
                .contains("Default label's result type is (result i64), but 1's is (result i32)")
        }
    }
    ...
    fun valid_ifStackFullWithI32Atop_andLabelHasEmptyResultType() = parser.with {
        val result = "br_table 0".parseInstruction()
            .validate(
                EMPTY_FUNCTION_BODY.prependLabel(ResultType(null))
    ...
    fun valid_ifStackSecond_matches_label() = parser.with {
        val result = "br_table 0".parseInstructions()
            .validate(
                BODY_WITH_LABEL
                    .pushStack(ValueType.I32)
                    .pushStack(ValueType.I32)
