 ...
 * * Then the instruction is valid with type `\[i32] => \[t]`.
 */
object LoadIntValidator : InstructionValidator<MemoryInstruction.LoadInt> {
    override fun validate(
        instruction: MemoryInstruction.LoadInt,
        ...
        validate(instruction.arg.isAlignmentValid(instruction.storageBytes), parseContext = null) {
            "Invalid alignment (${instruction.arg}) for N=${instruction.storageBits} " +
                "(alignment must not be larger than natural)"
        }
        state.popOperand(ValidationValueType.I32)

        val resultType = when (instruction.bitWidth) {
            32 -> ValueType.I32
            64 -> ValueType.I64
            else -> Impossible()
        }
