 ...
 */
object StoreFloatValidator : InstructionValidator<MemoryInstruction.StoreFloat> {
    override fun validate(
        instruction: MemoryInstruction.StoreFloat,
        context: ValidationContext.FunctionBody,
        state: InstructionSequenceState
    ) {
        context.validateMemoryExists()
        validate(instruction.arg.isAlignmentValid(instruction.byteWidth), parseContext = null) {
            "Invalid alignment for N=${instruction.byteWidth} (alignment must not " +
